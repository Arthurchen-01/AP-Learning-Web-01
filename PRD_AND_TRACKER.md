...
- [x] **P2.3 - 做题进度自动保存**：✅ 已完成（2026-04-10）
  - P1.2的`selectOption`在选答案时同步调用`saveState`，MCQ答题立即持久化
  - 刷新页面后答案不会丢失
  - 注：debounced自动保存为可选增强，当前实现已足够（P1.2测试已覆盖）

- [x] **P2.4 - 题目收藏/笔记功能**：✅ 已完成（2026-04-10）
  - 每道题下方添加笔记文本框（`#question-notes-input`）
  - 笔记内容 debounced（500ms）自动保存到 `section.notes[qIndex]`
  - 切换题目后笔记内容保持
  - 笔记随考试状态持久化到 localStorage
  - 3次连续测试 0 报错
